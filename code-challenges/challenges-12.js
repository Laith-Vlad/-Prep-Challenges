...
const cvsFiltration = (arr) => {
    ...
    arr.forEach(item => {
      ...
      if (item.yearsOfExperience > 4 && item.tech === 'JS') {
        fullName = item.firstName && item.LastName ? item.firstName + ' ' + item.LastName : item.firstName || item.LastName || '';
        output.push({ fullName, tech: item.tech });
      }
    });
    return output;
...
const vowelsFiltration = (arr) => {
    ...
    return arr.filter(arr => {
      return !vowels.some(vowel => arr.includes(vowel));
    });
  }
