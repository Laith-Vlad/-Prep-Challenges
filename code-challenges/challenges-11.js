...
function fullName(arr) {
    ...
    const fullNames = arr.map(fName => `${fName.firstName} ${fName.lastName}`);
    return fullNames;
  }
...
function gradesAvg(arr) {
    return arr.map(function(student) {
        ...
        let sum = student.gradsList.reduce(function(acc, curr) {
          return acc + curr;
        }, 0);
...
function studentsResult(arr) {
    return arr.map(student => {
        const avg = student.gradsList.reduce((acc, curr) => acc + curr, 0) / student.gradsList.length;
        const result = avg >= 50 ? 'Passed' : 'Failed';
        return { ...student, avg, result };
      });
